...
resubmit_to_table(struct xlate_ctx *ctx, struct dp_packet *packet_to_resubmit)
{
    ...
    /* could modify these technically
        res.in_port = 2;
        res.table_id = 0;
        res.with_ct_orig = false;
        look here compose_table_xlate
    */
    return ofproto_dpif_execute_actions__( ctx->xin->ofproto,
                                           version, &flow,
                                           NULL,
...
static struct dp_packet*
create_custom_packet_aggr(struct dp_packet *temp_pkt_for_flow, struct my_captured_packet *data, char** databuffer_p)
{
    struct dp_packet *packetAggr;
    packetAggr = dp_packet_new(1); //65535
...
*threadproc(void *arg)
{
    ...
    if(args->dict_entry->dp_packet_buff1[0].packet != 0 || args->dict_entry->dp_packet_buff1[0].packet != NULL) //overkill but needed
    {
        ...
        */
        ...
        struct dp_packet *packetAggr = create_custom_packet_aggr(args->dict_entry->dp_packet_buff1[0].packet,
...
compose_aggrs_action(struct xlate_ctx *ctx, struct dp_packet *packet_to_store,struct ofpact_aggrs *aggrs,  struct flow_dict *dict_entry, int flow_entry_index)
{
    ...
    if(dict_entry->index < PACKET_BUFF_ELEMENTS && dict_entry->argsForThread.timerstop == false && ctx->xin->packet)
    {
        ...
        VLOG_ERR("structure of packet added to buffer: %s ",  ofp_dp_packet_to_string(packet_to_store));
    ...
    if(dict_entry->index == PACKET_BUFF_ELEMENTS && dict_entry->argsForThread.timerstop == false && ctx->xin->packet)
    {
        ...
        char* a_p=NULL;
        char** buffer_data=&a_p;
        struct dp_packet *packetAggr = create_custom_packet_aggr(dict_entry->dp_packet_buff1[0].packet, dict_entry->dp_packet_buff1, buffer_data);
...
{
    int max = 0;
    struct ofport *port; //doesn't matter what this value is, saw it in OVS code somewhere
    HMAP_FOR_EACH(port, hmap_node, &ofprotodpif->up.ports)
    {
        if((int) port->ofp_port <= 65280)
        {
            max = ( max > (int) port->ofp_port ) ? max : (int) port->ofp_port;
        }
...
reassemble_message(struct my_split_packet *msp)
{
    ...
    for(int i=0; i < msp[0].tot_splits; i++)
    {
        ...
        //VLOG_INFO("split %.*s, and size_split: %d",size_split,(char *) dp_packet_get_udp_payload(msp[i].packet), size_split);
        //strncat(temp_payload, (char *) dp_packet_get_udp_payload(msp[i].packet), size_split); //this worked fine however with memcpy like in the tcp case, is better if we have payloads containing random null character
        memcpy(temp_payload + len, dp_packet_get_udp_payload(msp[i].packet),size_split);
        len += size_split;
    }
    VLOG_ERR("sto mandando questo %.*s ",msp[0].sizeofpayload,temp_payload);
    return temp_payload;
}
...
reassemble_tcp_message(struct my_tcp_split_packet *msp1)
{
    ...
    {
        memcpy(temp_payload_reass + len, dp_packet_get_tcp_payload(msp1[i].packet),tcp_payload_length(msp1[i].packet));
        len += tcp_payload_length(msp1[i].packet);
    }
...
{
    int rem = dp_packet_l4_size(packet_to_split) - UDP_HEADER_LEN; //starts as actual complete payload size, is decreased as splitting occurs
    int *split_arr = malloc((unsigned int)rem * sizeof(int));//this array will contain the random lengths of splits we'll apply to the payload,
...
generate_tcp_rand_split_array(struct dp_packet *packet_to_split)
{
    ...
    uint32_t *split_arr = (uint32_t*)malloc(rem * sizeof(uint32_t));//this array will contain the random lengths of splits we'll apply to the payload,
    //split_arr[0]='\0';
    // since we don't know how many splits we set the length of the array to the size of the payload
    int indexsplit = 0; //index to iterate over split_arr
    /* while length of original packet is > 0, keep splitting by a random number from 1 -> rem */
    ...
    //there are usually empty spots in splitarr
...
compose_split_tcp(struct xlate_ctx *ctx, uint16_t port)
{
    ...
    if(!eth_addr_equals(eth_pre_split->eth_src, fake_mac))
    {
        ...
        VLOG_ERR("tcpcountsplits: %d", tcpcountsplits);
        ...
        for(int i = 0; i < tcpcountsplits; i++)
        {
            ...
            VLOG_ERR("What I'm copying in split packet %.*s -",(int) split_arr[i], temp_payload);
            ...
            s_pkt.packet = create_custom_packet(packet_to_split, temp_payload, split_arr[i]); //insert split packet
            ...
            s_pkt.tcp_seq = tcph->tcp_seq;
            s_pkt.tcp_ack = tcph->tcp_ack;
            s_pkt.tcp_ctl = tcph->tcp_ctl;
            s_pkt.tcp_winsz = tcph->tcp_winsz;
            s_pkt.tcp_urg = tcph->tcp_urg;
            ...
            eth_hdr_for_splits->eth_dst = fake_mac;
            struct ip_header *iph = dp_packet_l3(pkt_to_send);
            char *strip = "99.99.99.99";
            ovs_be32 ip;
            ip_parse(strip, &ip);
            ovs_16aligned_be32 netip;
...
compose_split(struct xlate_ctx *ctx, uint16_t port)
{
    ...
    int truepayloadsize =  dp_packet_l4_size(packet_to_split) - UDP_HEADER_LEN; //-1 removes end of line char
    //VLOG_ERR("incoming packet of size %d to split: %.*s", truepayloadsize, truepayloadsize, complete_payload);
    ...
    for(int i = 0; i < countsplits; i++)
    {
        ...
        VLOG_ERR("What I'm copying in split packet %s -", temp_payload);
...
static int i_removed = 0; //index to check for empty slot in pseudo-dictionary
static struct csum_to_payload hold_to_rebuild[PACKETS_TO_HOLD] = {{0}}; //pseudo dictionary of 100 elements initialized to 0
static int pkts_to_rebuild = 0;
static void
compose_reass(struct xlate_ctx *ctx)
{
    VLOG_ERR("reassembling");
    struct eth_addr fake_mac = ETH_ADDR_C(44,34,56,78,9a,bc);
    struct eth_header *eth_pre_split = dp_packet_eth(ctx->xin->packet); //take ethernet header of incoming packet
    if(eth_addr_equals(eth_pre_split->eth_src, fake_mac))
    {
        ...
        VLOG_ERR("packets in array: %d", pkts_to_rebuild);
        if(i_csum >= 0) //if it exists
        {
            ...
            if( hold_to_rebuild[i_csum].arrived == s_pkt.tot_splits )
            {
                ...
                //VLOG_INFO("strlen of reass : %ld  sizeofpayload: %d, the payload: %s",  strlen(reass_payload), s_pkt.sizeofpayload,reass_payload);
        ...
        if( i_csum == -1 && pkts_to_rebuild < PACKETS_TO_HOLD) //if pkts_to_rebuild == 100 drop packets
        {
            ...
            VLOG_ERR("csumkey %"PRIu16, hold_to_rebuild[i_removed].csumkey);
            VLOG_ERR("i_removed %d", i_removed);
...
static int tcp_pkts_to_rebuild = 0; //counter to keep track of packets in the array hold_to_rebuild
static void
compose_reass_tcp(struct xlate_ctx *ctx)
{
    struct eth_addr fake_mac = ETH_ADDR_C(44,34,56,78,9a,bc);
    struct eth_header *eth_pre_split = dp_packet_eth(ctx->xin->packet); //take ethernet header of incoming packet
    if(eth_addr_equals(eth_pre_split->eth_src, fake_mac))
    {
        ...
        VLOG_ERR("tcp packets in array: %d", tcp_pkts_to_rebuild);
        if(i_csum >= 0) //if it exists
        {
            ...
            if(tcp_hold_to_rebuild[i_csum].arrived == s_pkt.tot_splits)
            {
                ...
                //char *chekapay = (char *) dp_packet_get_tcp_payload(split_recvd);
                ...
                struct tcp_header *tcph = dp_packet_l4(temp_packet);
                tcph->tcp_seq = tcp_hold_to_rebuild[i_csum].to_reassemble[0].tcp_seq;
                ...
                free(tcp_hold_to_rebuild[i_csum].to_reassemble);//tcp_hold_to_rebuild[i_csum].to_reassemble = NULL;
                //free(reass_payload_tcp);
        ...
        {
            VLOG_ERR("new packet with never before seen checksum");
            VLOG_ERR("csumkey %"PRIu16, tcp_hold_to_rebuild[i_tcpremoved].csumkey);
            VLOG_ERR("i_tcpremoved %d", i_tcpremoved);
